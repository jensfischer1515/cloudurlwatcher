...
public class QueueController {
    ...
    public String aggregateUrl(@RequestHeader(value = Headers.WATCH_URL, required = true) String watchUrl,
            @RequestHeader(value = Headers.WATCH_AGGREGATION, required = true) Schedule watchAggregation,
        ...
        UrlWatchResultAggregation urlWatchResultAggregation = urlWatchService.aggregateUrlWatchResults(watchUrl, new Interval(
                watchAggregateFrom, watchAggregateTo), watchAggregation);
        return String.format("Executed task '%s' from queue '%s'", taskName, queueName);
    }
